...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (product.seller_id === token.id) {
        return NextResponse.json(
          { error: 'Não é possível comprar seu próprio produto' },
          { status: 400 }
        );
      }
      ...
      const trxRes = await client.query(
        `INSERT INTO transactions 
         (product_id, buyer_id, seller_id, amount, platform_fee, seller_amount, payment_method, payment_provider, status)
         VALUES ($1, $2, $3, $4/100.0, $5/100.0, $6/100.0, $7, $8, $9)
         RETURNING *`,
        [
          product_id,
          token.id,
